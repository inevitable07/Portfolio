...
export default function NewProject() {
  ...
  return (
    <div className="max-w-2xl">
      <h2 className="text-xl font-bold mb-8">New Project</h2>

      <form onSubmit={handleSubmit} className="space-y-5">
        {/* Title */}
        <div>
          <label className="block text-xs text-white/40 mb-1.5">Title *</label>
          <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} required className={inputCls} placeholder="Project title" />
        </div>

        {/* Description */}
        <div>
          <label className="block text-xs text-white/40 mb-1.5">Description *</label>
          <textarea value={description} onChange={(e) => setDescription(e.target.value)} required rows={4} className={inputCls} placeholder="Describe the project..." />
        </div>

        {/* Tech Stack */}
        <div>
          <label className="block text-xs text-white/40 mb-1.5">Tech Stack (comma-separated)</label>
          <input type="text" value={techStack} onChange={(e) => setTechStack(e.target.value)} className={inputCls} placeholder="Next.js, React, Tailwind, ..." />
        </div>
